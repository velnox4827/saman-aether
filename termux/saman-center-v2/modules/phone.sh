...
s2_phone_api_test() {
    ...
    if s2_have termux-clipboard-get; then timeout 3 termux-clipboard-get >/dev/null 2>&1 && s2_ok "Clipboard API responds" || s2_warn "Clipboard API unavailable/denied"; fi
...
s2_phone_menu() {
    while true; do
        ...
        case "$x" in
            ...
            2) termux-torch on 2>/dev/null && s2_ok "Flashlight ON" || s2_err "Could not turn flashlight on"; sleep 1 ;;
            3) termux-torch off 2>/dev/null && s2_ok "Flashlight OFF" || s2_err "Could not turn flashlight off"; sleep 1 ;;
            4) echo; termux-clipboard-get 2>/dev/null || s2_err "Clipboard unavailable"; s2_pause ;;
            5) read -r -p "Text: " txt; printf '%s' "$txt" | termux-clipboard-set 2>/dev/null && s2_ok "Copied" || s2_err "Clipboard unavailable"; sleep 1 ;;
            6) s2_notify "Saman Phone Center" "Termux API is working ✓"; termux-vibrate -d 150 >/dev/null 2>&1 || true; s2_ok "Notification requested"; sleep 1 ;;
            7) s2_phone_api_test; s2_pause ;;
            8) read -r -p "Server IP / hostname: " host; [ -z "$host" ] && continue; read -r -p "Username: " user; read -r -p "Port [22]: " port; port="${port:-22}"; if [ -n "$user" ]; then ssh -p "$port" "$user@$host"; else ssh -p "$port" "$host"; fi ;;
            0) return ;;
        esac
    done
}
